...
class RDSPanelStorage:
    ...
    def __init__(
        ...
        self.data_source_id = os.getenv("KB_DATA_SOURCE_ID")
    ...
    def save_panel_result(
        self, result, country_code: str, skip_s3: bool = False
    ) -> int:
        """
    ...
    def _insert_analysis(
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql, (discussion_id, expert_role, str(analysis_text), round_number)
                )
                conn.commit()
        finally:
            conn.close()
    ...
    def _save_country_sentiment(self, result_dict: Dict, country_code: str):
        ...
        if country_id is None:
            print(
                f"⚠️  Country '{country_code}' not found in insights_country table. Skipping CountrySentiment save."
            )
            return
        ...
        try:
            with conn.cursor() as cur:
                ...
                conn.commit()
                print(
                    f"   ✅ CountrySentiment: {label} ({score}/100) for country_id={country_id}"
                )
        except Exception as e:
...
"""
        ...
        for i, article in enumerate(news_articles, 1):
            sentiment_label = (
                "Positive"
                if article.get("sentiment", 0) > 0.2
                else "Negative" if article.get("sentiment", 0) < -0.2 else "Neutral"
            )
...
"""
    ...
    def _save_weather_to_s3_for_kb(self, country_code: str, weather: Dict):
        ...
        mood_label = (
            "Positive"
            if weather.get("mood_impact", 0) > 0.2
            else "Negative" if weather.get("mood_impact", 0) < -0.2 else "Neutral"
        )
...
"""
    ...
    def _trigger_kb_ingestion(self):
    ...
    def list_discussions(
        self, country_code: Optional[str] = None, limit: int = 20
    ) -> List[Dict]:
...
def create_rds_storage(**kwargs) -> RDSPanelStorage:
    ...
    return RDSPanelStorage(
        ...
        or os.getenv("KB_S3_BUCKET", "hackthon-knowledge-base"),
